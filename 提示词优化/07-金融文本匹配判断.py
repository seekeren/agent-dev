...
# for key, value in examples_data.items():
#     messages.append({"role": "user", "content": "句子1是:[" + " " + "]句子2是".join(value[0])})
#     messages.append({"role": "assistant", "content": key})
#     messages.append({"role": "user", "content": "句子1是:[" + " " + "]句子2是".join(value[1])})
#     messages.append({"role": "assistant", "content": key})


for q in questions:
    response = client.chat.completions.create(model="qwen3.6-flash-2026-04-16",
                                              messages=messages + [
                                                  {"role": "user", "content":  f"判断是否匹配。[{q[0]}] 和 [ {q[1]} ] "}])
